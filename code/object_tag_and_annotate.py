...
IMAGE_DIR = "data_set_no_ball/images"
ANNOTATION_DIR = "data_set_no_ball/annotations"
IM_WIDTH = 640  
IM_HEIGHT = 480  
...
class RaspberryPiObjectDetection:  
    def __init__(self, model_path, model_labels, confidence_threshold=0.6):  
        ...
        try:  
            ...
            self.images_dir = BASE_DIR + "/" + IMAGE_DIR
            self.annotation_dir = BASE_DIR + "/" + ANNOTATION_DIR
            self.image_count = 0
    ...
    def save_annotation(self, image_path, bbox):  
        ...
        # Write annotation (class 90 for ball, followed by normalized coordinates)  
        with open(annotation_path, 'w') as f:  
            f.write(f"90 {x} {y} {w} {h}")  
    ...
    def detect_objects(self, image):  
        ...
        try:  
            ...
            key = input ("Enter key - c to continue")
            print ("got %s"  % (key))
            ball_bbox = None
            if (len(detections) > 0) :
                ball_bbox = detections[0]['box']
            else:
                print (" 0 detections...")
            if key == "c" :  
                print ("writing image details.....")
                # Generate unique filename  
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")  
                image_filename = f'ball_{timestamp}.jpg'  
                ...
                # Save annotation  
                #self.save_annotation(image_filename, ball_bbox)  
    ...
    try:  
        ...
        try:  
            while running:  
                ...
                # Detect objects  
                proceed,detections = detector.detect_objects(image)  
