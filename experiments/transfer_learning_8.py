...
class TennisBallDetector:  
    ...
    def create_model(self, trainImages):  
        ...
        #total_images=trainImages ,  # Total training images  
    ...
    def prepare_labels(self, y_train, y_val):
        # Convert to 1D and int32 for sparse_categorical_crossentropy
        #y_train_classification = np.array(y_train['class_output']).reshape(-1).astype("int32")
        #y_val_classification = np.array(y_val['class_output']).reshape(-1).astype("int32")

        # Make sure bounding_box is shape (N,4)
        y_train_coordinates = np.array(y_train['coordinate_output'])  # (N, 4)
        y_val_coordinates   = np.array(y_val['coordinate_output'])    # (N, 4)

        y_train_prepared = {
            "class_output": y_train['class_output'],
            "coordinate_output":  y_train_coordinates
        }
        y_val_prepared = {
            "class_output": y_val['class_output'],
            "coordinate_output":  y_val_coordinates
        }
    ...
    def train(self, X_train, y_train, X_val, y_val):  
        ...
        # Diagnostic prints  
        #print("Reshaped Training Classification Labels Shape:", y_train['class_output'].shape)  
        #print("Reshaped Training Bounding Box Labels Shape:", y_train['coordinate_output'].shape)  
        

        #y_train_classification = y_train["class_output"]
        #y_train_coordinates = y_train["coordinate_output"]
        print("Training Classification Labels Shape:", len(y_train['class_output']))  
        print("Training Bounding Box Labels Shape:", len(y_train['coordinate_output']))  
        ...
        #assert y_train['class_output'].shape[0] == y_train['coordinate_output'].shape[0], "Mismatch in number of samples"  
    ...
    def inference_and_visualize(self, X_val, y_val):  
        ...
        for i in range(min(30, len(X_val))):  
            ...
            if true_class == 0:  # True ball detected  
                ...
                # Use the current image shape for scaling  
                x_true = int(x_true * img_cv.shape[1])  
                y_true = int(y_true * img_cv.shape[0])  
                
                # Adjust radius calculation  
    ...
    def prepare_for_model(self, images):  
        """  
    ...
    def ensure_same_shape(self, images, classification_labels, bounding_boxes):  
        """  
...
def basic_split_data(images, labels, coordinates, train_split=0.9):  
        """  
        Split data into train and validation sets  
        """  
        # Shuffle data  
...
def basic_train(model, X_train, y_train, X_val, y_val):  
        # Early stopping and model checkpoint  
        early_stopping = tf.keras.callbacks.EarlyStopping(  
            monitor='val_loss',   
            patience=10,   
...
def main():  
    ...
    #combined = list(zip(augmented_images, augmented_coordinates, augmented_validity))

    # Shuffle the combined list
    #random.shuffle(combined)

    # Unzip the shuffled list back into separate lists
    #augmented_images, augmented_coordinates, augmented_validity = zip(*combined)
    # Shuffle the indices  
    ...
    # Reorder the lists using the shuffled indices  
    #augmented_images = [augmented_images[i] for i in indices]  

    #print (type(augmented_coordinates))

    #augmented_coordinates = [augmented_coordinates[i] for i in indices]  
    #augmented_validity = [augmented_validity[i] for i in indices]  
    augmented_images = detector.resize_images_cv2(augmented_images)
    

    #print (len(augmented_images))
    #print (len(augmented_coordinates))
    #print (len(augmented_validity))

    #augmented_images = detector.prepare_for_model(augmented_images)

    #visualize_annotations(augmented_images[:10], augmented_coordinates[:10])  

    #detector.ensure_same_shape(augmented_images,   augmented_validity,   augmented_coordinates  )
