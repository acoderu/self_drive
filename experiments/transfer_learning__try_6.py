...
class TennisBallDetector:
    ...
    def create_model(self):  
        ...
        for layer in base_model.layers[:-4]:  # Unfreeze the last 10 layers  
            layer.trainable = False  

        inputs = tf.keras.Input(shape=self.input_shape)  
        x = base_model(inputs)  
        x = tf.keras.layers.Conv2D(256, (3, 3), activation='relu6', padding='same')(x)  
        x = tf.keras.layers.BatchNormalization()(x)  
        x = tf.keras.layers.MaxPooling2D((2, 2))(x)  
        x = tf.keras.layers.Flatten()(x)  # Replace GlobalAveragePooling2D  

        # Classification branch  
        x_class = tf.keras.layers.Dense(256, activation='relu6')(x)  
        x_class = tf.keras.layers.Dropout(0.3)(x_class)  # Reduce dropout  
        ...
        # Localization branch  
        x_loc = tf.keras.layers.Dense(128, activation='relu6')(x)  # Simplify  
        x_loc = tf.keras.layers.Dropout(0.3)(x_loc)  # Reduce dropout  
        ...
        # Create the final quantized model  
        ...
        model.compile(  
            ...
            loss={  
                ...
                'coordinate_output': tf.keras.losses.Huber()  # Try Huber Loss  
            },  
            loss_weights={  
                'class_output': 1.0,  
                'coordinate_output': 5.0  
            },  
            metrics={  
    ...
    def save_tflite_model(self, images, filename="tennis_ball_detector_1.tflite"):
        ...
        try:  
            ...
            converter.inference_output_type = tf.float32  # Match output type  
