import os
# Set TF_USE_LEGACY_KERAS to False (or 0)  
...
class TennisBallDetector:
    ...
    def create_model(self, train_images_count):
        ...
        # Convert all ReLU6 activations to ReLU (TPU-Compatible)
        def replace_relu6_with_relu(layer):
            if hasattr(layer, "activation") and layer.activation == tf.keras.activations.relu6:
                layer.activation = keras.activations.relu

        for layer in base_model.layers:
            replace_relu6_with_relu(layer)

        # Freeze MobileNetV2 Backbone (Efficient for Edge TPU)
        base_model.trainable = False  

        inputs = tf.keras.Input(shape=self.input_shape)
        x = base_model(inputs)

        # Use TPU-optimized Depthwise Separable Convolution instead of Conv2D
        x = tf.keras.layers.SeparableConv2D(256, (3, 3), activation='relu', padding='same')(x)
        x = tf.keras.layers.MaxPooling2D((2, 2))(x)
        x = tf.keras.layers.GlobalAveragePooling2D()(x)

        # Classification branch
        x_class = tf.keras.layers.Dense(256, activation='relu')(x)
        ...
        # Localization branch
        x_loc = tf.keras.layers.Dense(256, activation='relu')(x)
        ...
        # Create the model
        model = tf.keras.Model(inputs=inputs, outputs=[classification_output, coordinate_output])
        return model
    ...
    def train(self, X_train, y_train, X_val, y_val):  
        ...
        self.model.compile(  
            ...
            loss={  
                ...
                'coordinate_output': 'mean_squared_error'  
            },  
            loss_weights={  
                'class_output': 2.0,  
                'coordinate_output': 3.0  
            },  
            metrics={  
    ...
    def save_tflite_model(self, images, filename="tennis_ball_detector_1.tflite"):
        ...
        def preprocess_image(image):  
            # Resize to model's input size (e.g., 224x224)  
            #resized_image = cv2.resize(image, (224, 224))  

            # Normalize to [0, 1] (if the model expects normalized inputs)  
            #input_data = resized_image.astype(np.float32) / 255.0  

            # Convert to uint8 (if the model is quantized to INT8)  
        ...
        def representative_data_gen():  
            for image in images:  # Replace with your dataset  
        ...
        try:  
            ...
            converter.target_spec.supported_ops = [tf.lite.OpsSet.TFLITE_BUILTINS_INT8]  # Avoid SELECT_TF_OPS  
            converter.inference_input_type = tf.uint8  # Match input type  
            converter.inference_output_type = tf.uint8  # Match output type  
...
def main():
    ...
    #detector.model = tfmot.quantization.keras.quantize_model(detector.model)  
