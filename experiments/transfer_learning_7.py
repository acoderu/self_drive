...
def main():  
    ...
    labels = []
    labels.append(label)
    augmented_labels = np.repeat(labels, len(images), axis=0)  
    
    #X_train, y_train, X_val, y_val = basic_split_data(  
    #    images,   
    #    augmented_labels,   
    #    coordinates  
    #)  
    ...
    # Train model  
    #history = basic_train(model, X_train, y_train, X_val, y_val)  
    ...
    #print(f"X train sz={len(X_train)}, Y train={len(y_train['class_output'])}")   
    #print(f"X test sz={len(X_val)}, Y test={len(y_val['class_output'])}")   
    ...
    # Train model  
    #history = basic_train(model, X_train, y_train, X_val, y_val)  
    #history = basic_train(detector.model, X_train, y_train, X_val, y_val)  

    #detector.model = detector.create_model()
    #print(detector.model.output_names)
    # Train model  
